...
\begin{document}
\title{A Binary-Image Transition State Search Method}
...
A reduction factor of $f = 0.5$ is succesful for most applications, but this can be made smaller to ensure that the states do not slide off the ridge between the two basin of attraction.
...
Each of these are modelled using very different systems with \num{14}, \num{35400}, and \num{40000} degrees of freedom, respectively.
The details about how they are modelled are included in the methods section.
...
The method is shown to be more efficient than existing double-ended methods, both in memory and computational cost, and we have demonstrated that it is possible to utilise adaptive remeshing techniques to further reduce the computational cost.
