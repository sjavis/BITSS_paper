...
\begin{abstract}
  ...
  \hl{We also snow it is more robust compared to existing bracketing methods that use only two states.}
...
Furthermore, \hl{using both an energy and distance constraint with BITSS provides improved efficiency over these methods which each use just one of the constraints.}
...
Although reliable, chain-of-states approaches \hl{may need to use a large number of states in cases where the transition pathway is highly complex or non-linear.
...
In contrast, it is unneccessary to assume any initial interpolations for the BITSS method.}
