...
However, because the BITSS method uses just two states, it has the potential to be significantly faster in these cases.}
This is tested here by measuring convergence to the transition state for BITSS in comparison with the string method \cite{E2007} and DNEB \cite{Trygubenko2004} using 5, 10, and 20 images \hl{(including the endpoints, which are fixed in place at the minima).
These methods are often used in conjuction with another method to accurately find the transition state, although we will first examine just the basic methods.
...
\hl{The results in {\cref{fig:speedtest}}, demonstrate one main advantage for the BITSS method: it is able to accurately approximate the transition state without the need for a hybrid method.
In contrast, the chain-of-states are several orders of magnitude less accurate at obtaining the transition state configuration.
In particular, in the highly non-linear wetting example they do not significantly improve upon the initial pathway estimate.
However, it is worth noting that while these do not converge to the transition state, they do converge to the minimum energy pathway (or a close approximation of it).
This convergence occurs relatively quickly and causes the lines to plateau in {\cref{fig:speedtest}}.}

\hl{To accurately obtain the transition state the chain-of-states methods are often used in conjuction with another method.
Climbing image hybrid methods {\cite{Henkelman2000a,E2007}} are often used, but if the pathway is non-linear these can require a large number of images.
Hybrid eigenvector-following methods {\cite{Cerjan1981}} are another commonly used approach, starting from the state with the highest on the chain.
...
For completeness, we also use a hybrid eigenvector-following with BITSS.
For this we set the criterion for switching to the eigenvector-following based upon the size of the change in the average of the two states over a BITSS step.}
...
\begin{table}[]
  ...
  \caption{\label{tab:speed}
    ...
    The DNEB hybrid eigenvector-following has been run for different numbers of images.
    Convergence is determined as when the root-mean-square of the gradient is less than $10^{-4}$.
    }
  }
\end{table}

\hl{We have compared the speed of convergence to the transition state for BITSS, BITSS with hybrid eigenvector-following, and DNEB with hybrid eigenvector-following.
The results of which are shown in \mbox{\cref{tab:speed}}.
We see that for these examples, the speed of BITSS with hybrid eigenvector-following does not significantly improve upon the speed of BITSS alone.
Indeed, for the wetting example it is considerably slower.
Therefore, it is probably reasonable to use the basic BITSS method for many applications.
Secondly, it is shown that DNEB with a small number of images is the fastest method for the Lennard-Jones cluster, whereas for the droplet wetting DNEB is considerably slower even with a small number of images.
