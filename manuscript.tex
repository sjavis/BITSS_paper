...
BITSS is advantageous in both these regards, as only two states are evolved, regardless of the pathway complexity, and no initial interpolation is required.
For these advantages to be of practical use, BITSS must demonstrate at least a comparable speed of convergence to the transition states, relative to these hybrid chain-of-states methods, which we now investigate.
...
Alternatively, DNEB-HEVF involves first minimising the chain of states until a criterion is met, and then performing eigenvector-following from the highest energy state, moving uphill along the smallest eigenvector of the Hessian until it reaches the transition state.
It is also possible to combine eigenvector-following with BITSS, which we also include in the comparison for completeness.
}{\color{blue}
...
The spring constants connecting the states in CINEB and DNEB are system dependent such that they keep the states equidistant without overwhelming the single state forces.
The parameters chosen for this are $10^{-1}$ for the particle cluster, $10^{-2}$ for cylindrical buckling, and $10^{-6}$ for the striped wetting.

In the hybrid eigenvector-following methods, criteria are used to determine when the initial method has converged sufficiently, at which point the eigenvector following begins.
For BITSS, the criterion is when the average of the two states changes during a BITSS step by less than a tenth of the reduction in the separation given by \cref{eq:diteration}.
As for DNEB, the root-mean-square of the total energy gradient of the chain of states is used with a convergence criterion of $10^{-3}$ for the particle cluster and cylindrical buckling systems, or $10^{-5}$ for the striped wetting.
The eigenvector-following method itself was performed using the program OPTIM \cite{OPTIM}.
}
