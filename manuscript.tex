...
Move this to methods: For this, the switch from BITSS to eigenvector-following is chosen to occur when the average of the two states changes by less than a tenth of the reduction in the distance given by \cref{eq:diteration}.
}\hl{%
...
As is shown for cylindrical buckling with three images, efficiently finding the TS is challenging for both CINEB and DNEB-HEVF.
BITSS however converges using only two images.

However, if too few images are used (the minimum number of which is system dependant), both CINEB and DNEB-HEVF can struggle, as observed for the cylindrical buckling with three images.
}
...
{\color{red}
\section{Implementation of the hybrid methods, CINEB, DENB-HEVF, and BITSS-HEVF}
JRP - Sam can you fill out this section to briefly detail the method parameters?
Also, say that HEVF was performed using the program OPTIM, with the reference http://www-wales.ch.cam.ac.uk/software.html.
}
