...
\begin{figure}[htb]
  ...
  \caption{\label{fig:bracketing}
    ...
    The equation for this potential is provided in \cref{sec:bracketingpotential}.
    The minimum energy pathway is shown by the dashed line.
  }
\end{figure}
...
\begin{figure*}[tb]
  \includegraphics[width=\linewidth]{testsystems.pdf}
  \caption{\label{fig:testsystems}
    Comparisons between BITSS, DNEB, and the string method in the distance from the transition state as a function of the number of calculations of the energy gradient.
...
Although reliable, chain-of-states approaches may need to use a large number of states in cases where the transition pathway is highly complex or non-linear.
Since the BITSS method uses just two states, it has the potential to be more efficient in these cases.
...
{\Cref{fig:testsystems}} shows the comparison between BITSS, DNEB and the string method.
...
The spring constants that have been used are $0.1$, $0.01$, and $10^{-4}$ for the three respective systems.

In \cref{fig:testsystems}, for each system, BITSS quickly converges to the transition state.
...
It is interesting to note that for the high-dimensional, non-linear pathways of wetting example shown in \cref{fig:testsystems}c, DNEB and the string method do not substantially reduce the distance to the transition state compared to the initial interpolation.

To accurately obtain the transition state, the chain-of-states methods are often used in conjunction with another refinement method.
...
For example, in the wetting example of \cref{fig:testsystems}c, we find even an 100-image initial double-ended search is not sufficient for the climbing image method to then successfully refine the transition state.
Hybrid eigenvector-following (HEVF) methods \cite{Cerjan1981} are another commonly used approach, starting from the state with the highest energy on the chain.
...
In \cref{tab:speed}, we compare the speed of convergence to the transition state for BITSS, BITSS with hybrid eigenvector-following, and DNEB with hybrid eigenvector-following.
...
We see that, for the examples in \cref{tab:speed}, the speed of BITSS with hybrid eigenvector-following does not significantly improve upon the speed of BITSS alone.
...
Despite the large number of degrees of freedom, this buckling mode in \cref{fig:testsystems}c is known to exhibit a linear pathway close to the transition state, being controlled simply by the dimple depth increasing\cite{Panter2019}.
...
The 2D potential used for the comparison of bracketing methods in \cref{fig:bracketing} uses a sum of Gaussian potentials, \cref{eq:bracketingparams}, the parameters for which are provided in \cref{tab:bracketingparams}.
\begin{equation}
