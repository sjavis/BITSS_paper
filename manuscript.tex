...
\temp{Indeed, the only circumstance under which a zero-mode could cause BITSS to fail is if it is positioned at the transition state in the direction of the pathway.
In this case there is no single transition state, instead the minimum energy pathway would have a flat section at the top of its energy profile.}
