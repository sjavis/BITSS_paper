...
Flat landscapes can pose issues for single-ended search methods that rely only upon local information, and current methods cannot typically be applied to discontinuous landscapes because they require continuous, differentiable optimisation functions.
...
This demonstrates that BITSS is able to converge to the transition state so long as the remeshing does not effectively change the landscape.
...
However, for some applications it may be sufficient to use simpler distance measures, such as the difference between average values of the system.
...
When only a single state has a zero-gradient mode then the other is likely to slide down the potential slightly (iii), but the two states still remain either side of the dividing ridge and so the result will be unaffected.
...
However, BITSS can be used for these systems with undefined gradients with just a couple of alterations.
