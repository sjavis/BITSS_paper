...
It is interesting to note that for the high-dimensional, non-linear pathways of wetting example shown in {\cref{fig:speedtest}}c, DNEB and the string method do not substantially reduce the distance to the transition state compared to the initial interpolation.}
...
For this, the criterion for switching to the eigenvector-following is when the change in the average of the two states is less than a tenth of the distance reduction during a BITSS step.}
...
Secondly, it is shown that DNEB with a small number of images is the fastest method for the Lennard-Jones cluster, and an intermediate number of images is optimal for the buckling because if there are too few images it will not converge to the correct transition state.
On the other hand, for the droplet wetting DNEB is considerably slower, even with a small number of images.
This is in agreement with our expectation that BITSS offers an efficiency advantage for complex pathways.}
...
\subsection{Hook potential equation}\label{sec:bracketingpotential}
\hl{
