...
\hl{These typically involve changing the discretisation of continuous systems to focus the computational time on important regions, for example, using a higher resolution mesh in regions of high stress in finite element simulations {\cite{Lee1994}}.
...
\begin{figure}[htb]
  ...
  \caption{\label{fig:bracketing}
    ...
    The minimum energy pathway is shown by the dashed line.
    {\color{green}Sam, can you put the equation for this landscape in the methods section? }
  }
\end{figure}
...
In \hl{the} step and slide method, the separation between two images is minimised while their energy is fixed (iteratively increasing the energy up to the transition state).
...
This is because the combination of distance and energy constraints allow BITSS to approach a transition state from below and above (by sliding down a ridge).}

Furthermore, \hl{using both an energy and distance constraint with BITSS provides improved efficiency over these methods which each use just one.}
...
\hl{Also, restricting the minimisation to a constant energy surface can result in a considerably more complex method, as the states must be constantly projected back onto this surface.}


\subsection{\hl{Comparison with chain-of-states methods}}
\begin{figure*}[tb]
  \includegraphics[width=\linewidth]{speedtest.pdf}
  \caption{\label{fig:speedtest}
    Comparisons between BITSS, DNEB, and the string method in the \hl{distance from} the transition state as a function of the number of calculations of the energy gradient.
...
Although reliable, chain-of-states approaches \hl{may need to use tens of states for systems if the transition pathway is highly complex or non-linear.
However, because the BITSS method uses just two states, it has the potential to be more efficient in these cases.
This is analysed here in two ways.
First, we measure how quickly BITSS reduces the distance to the transition state, compared to the string method {\cite{E2007}} and DNEB {\cite{Trygubenko2004}} using 5, 10, and 20 images (including the endpoints, which are fixed in place at the minima).
Second, these methods are often used in conjunction with another method to accurately find the transition state.
Here we compare the number of potential gradient calculations required to reach the transition state for BITSS, BITSS in combination with hybrid eigenvector-following, and DNEB in combination with hybrid eigenvector-following.
...
\hl{{\Cref{fig:speedtest}} shows the comparison between BITSS, DNEB and the string method.
Here, the $x$-axis corresponds to the number of calculations of the potential gradient.
The measure in the $y$-axis is the distance from the true transition state.}
...
The parameters used in the chain-of-states methods are chosen such that they optimise the speed of convergence.
For the string method, this is the number of downhill iterations before the images are redistributed.
The optimal values are found to be 2, 50, and 20 for the particle cluster, cylindrical buckling, and striped wetting examples, respectively.
In the case of DNEB, the spring constant between the images is optimised instead.
\hl{The spring constants that have been used are $0.1$, $0.01$, and $10^{-4}$ for the three respective systems.}

\hl{In {\cref{fig:speedtest}}, for each system, BITSS quickly converges to the transition state.
In contrast, DNEB and the string method converge quickly to an approximation of the minimum energy pathway.
However, as the images are spaced along the chain of states, it is unlikely that they closely approach the transition state.
It is interesting to note that for the high-dimensional, non-linear pathways of wetting example shown in {\cref{fig:speedtest}}c, DNEB and the string method to not substantially reduce the distance to the transition state compared to the initial interpolation.}

\hl{To accurately obtain the transition state, the chain-of-states methods are often used in conjuction with another refinement method.
One common set of methods are the climbing image hybrid methods {\cite{Henkelman2000a,E2007}}, but if the pathway is non-linear these can require a large number of images.
For example, in the wetting example of {\cref{fig:speedtest}}c, we find even an 100-image initial double-ended search is not sufficient for the climbing image method to then successfully refine the transition state.
Hybrid eigenvector-following methods {\cite{Cerjan1981}} are another commonly used approach, starting from the state with the highest energy on the chain.
In this approach the eigenvector with the smallest eigenvalue of the Hessian is found, along which direction the state is then moved uphill until eventually it reaches the transition state.
For completeness, we also use a hybrid method combining eigenvector-following with BITSS.
For this, the criterion for switching to the eigenvector-following is when the change in the average of the two states is less than half of the distance reduction during a BITSS step.}

\begin{table}[]
  \begin{tabular}{ccccccc}
    \hline\hline
    \multirow{2}{*}{System} & \multirow{2}{*}{BITSS} &
    \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}BITSS-\\HEVF\end{tabular}} &
    \multicolumn{4}{c}{DNEB-HEVF} \\
             &             &             & 3           & 5           & 10          & 20          \\
    \hline
    LJ-7     & 148         & 140         & 135         & 138         & 208         & 361         \\
    Wetting  & \num{12100} & \num{15537} & \num{17840} & \num{17721} & \num{20221} & \num{48286} \\
    \hline\hline
  \end{tabular}
  \caption{\label{tab:speed}
    \hl{
    Number of potential gradient calculations required to reach the transition state for the Lennard-Jones cluster and the droplet wetting examples.
    The approach combining DNEB and hybrid eigenvector-following (HEVF) has been run for different numbers of images.
    Convergence is determined as when the root-mean-square of the gradient is less than $10^{-4}$.} {\color{green} Need to add the buckling result.}
  }
\end{table}
...
{\color{green}Secondly, it is shown that DNEB with a small number of images is the fastest method for the Lennard-Jones cluster, whereas for the droplet wetting DNEB is considerably slower, even with a small number of images.
This agrees with our expectation that BITSS offers an efficiency advantage for complex pathways.
WE NEED TO UPDATE THIS DISCUSSION WHEN WE HAVE THE BUCKLING RESULT.}
...
\begin{figure}[tb]
  ...
  \caption{\label{fig:adaptivemesh}
    ...
    (a) Snapshots of the BITSS method for the buckling of a cylinder with a changing mesh.
        The radial displacement \hl{relative to the} unbuckled cylinder is shown, as well as the underlying triangular mesh.
